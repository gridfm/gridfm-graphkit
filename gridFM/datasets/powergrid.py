...
class GridDatasetMem(InMemoryDataset):
    ...
    def process(self):
        ...
        for idx in tqdm(range(self.max_scenarios)):
            ...
            # mask value is -1
            x[:, : self.mask_dim][mask] = self.mask_value
...
class GridDataset(Dataset):
    ...
    def processed_file_names(self):
        return [
            f"data_{self.norm_method}_{idx}.pt" for idx in range(self.max_scenarios)
        ]
    ...
    def process(self):
        ...
        for idx in tqdm(range(self.max_scenarios)):
            ...
            torch.save(
                graph_data,
                osp.join(self.processed_dir, f"data_{self.norm_method}_{idx}.pt"),
            )
