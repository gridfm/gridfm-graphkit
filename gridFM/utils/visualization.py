...
def visualize_error(data_point, model, baseMVA, device):
    ...
    with torch.no_grad():
        ...
        data_point.x[:, : data_point.mask.shape[1]][data_point.mask] = mask_value_expanded[data_point.mask]
        output = model(data_point.x, data_point.pe, data_point.edge_index, data_point.edge_attr, torch.zeros(data_point.x.shape[0], dtype=int).to(device))
        loss_dict = loss(output, data_point.y, data_point.edge_index, data_point.edge_attr, data_point.mask)
        active_loss = loss_dict["Nodal Active Power Loss in p.u."]
        active_loss = active_loss.cpu() * baseMVA
    
    # Create a graph
    G = nx.Graph()
    edges = [(u, v) for u, v in zip(data_point.edge_index[0].tolist(), data_point.edge_index[1].tolist()) if u != v]
    ...
    for node_type, shape in node_shapes.items():
        ...
        nx.draw_networkx_nodes(
            G, pos, nodelist=nodes, node_color=[active_loss[i] for i in nodes], 
            cmap=cmap, node_size=800, ax=ax, vmin=vmin, vmax=vmax, node_shape=shape
        )
    ...
    nx.draw_networkx_labels(G, pos, labels=node_labels, font_size=10, font_color="white", font_weight="bold",ax=ax)
...
def visualize_quantity_heatmap(data_point, model, quantity, quantity_name, unit, node_normalizer, cmap, device):
    """
    ...
    with torch.no_grad():
        ...
        predicted_values[~data_point.mask[:, quantity]] = denormalized_gt[~data_point.mask[:, quantity], quantity]
    ...
    for node_type, shape in node_shapes.items():
        ...
        nx.draw_networkx_nodes(
            G, pos, nodelist=nodes, node_color=[gt_values[i] for i in nodes], 
            cmap=cmap, node_size=node_size, ax=ax, vmin=vmin, vmax=vmax, node_shape=shape
        )
    ...
    nx.draw_networkx_nodes(G, pos, nodelist=masked_node_indices, node_color='#D3D3D3', node_size=750, ax=ax)
    nx.draw_networkx_edges(G, pos, edge_color='gray', alpha=0.5, ax=ax, width=2)
    nx.draw_networkx_labels(G, pos, labels=node_labels, font_size=10, font_color="white", font_weight="bold", ax=ax)
    ...
    for node_type, shape in node_shapes.items():
        ...
        node_size = 390 if node_type == 'REF' else 600
        nx.draw_networkx_nodes(
            G, pos, nodelist=nodes, node_color=[predicted_values[i] for i in nodes], 
            cmap=cmap, node_size=node_size, ax=ax, vmin=vmin, vmax=vmax, node_shape=shape
        )
