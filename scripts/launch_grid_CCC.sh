...
if [ -n "$GRID_CONFIG_FILE" ] && [ -n "$CONFIG_FILE" ]; then
    echo "Running $PYTHON_SCRIPT with confif: $CONFIG_FILE and additional grid configuration: $GRID_CONFIG_FILE"
    python "$PYTHON_SCRIPT" --config "$CONFIG_FILE" --grid "$GRID_CONFIG_FILE" 
else
