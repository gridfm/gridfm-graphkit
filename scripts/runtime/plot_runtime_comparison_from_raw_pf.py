#!/usr/bin/env python3
"""Regenerate the paper's s1 PF runtime comparison from raw CSV files."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--genco-root", type=Path, default=GENCO_MATRIX_ROOT)
    parser.add_argument(
        "--powermodels-root",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=PAPER_FIGURE_ROOT / "runtime_comparison_from_raw_pf.pdf",
    )
...
def main() -> None:
    ...
    powermodels_best: dict[str, list[float]] = {"pf": [], "dcpf": []}
    ...
    # AC-PF uses NLsolve through GOC 500 and JuMP on GOC 2000/10000.
    # Plot as two solid green series (distinct colors); DC-PF stays continuous.
    ac_pf_split = bus_counts.index(500) + 1
    pm_pf_nlsolve_line = axis.plot(
        bus_counts[:ac_pf_split],
        powermodels_best["pf"][:ac_pf_split],
        color="#2ca02c",
        linestyle="-",
        marker="D",
        markersize=6,
    ...
    pm_dcpf_line = axis.plot(
        bus_counts,
        powermodels_best["dcpf"],
        color="#2ca02c",
        linestyle=":",
        marker="D",
        markersize=6,
    ...
    pm_legend = axis.legend(
        handles=[pm_pf_nlsolve_line, pm_pf_jump_line, pm_dcpf_line],
        title="PowerModels",
        loc="upper left",
